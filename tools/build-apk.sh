...
SDK_BASENAME="openwrt-sdk-${OPENWRT_VERSION}-${SDK_ARCH}_gcc-14.3.0_musl.Linux-x86_64"
...
SDK_SHA256="${SDK_SHA256:-9f371906ce6d2f95418f69fac06df58bf9df0ffc4abed6206c30f1b547fcfc12}"

ROOT_DIR="$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)"
CACHE_DIR="${CACHE_DIR:-$ROOT_DIR/work}"
WORK_DIR="${WORK_DIR:-/tmp/luci-app-campus-portal-sdk}"
...
APK_FILE="$(find "$SDK_DIR/bin/packages" -type f -name 'luci-app-campus-portal-*.apk' | head -n 1)"

if [ -z "$APK_FILE" ]; then
	echo "Build succeeded but no APK was found" >&2
	exit 1
fi

cp "$APK_FILE" "$OUTPUT_DIR/"
echo "APK: $OUTPUT_DIR/$(basename "$APK_FILE")"
